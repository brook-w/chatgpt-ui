...
<template>
  <v-app :theme="$colorMode.value">
    <v-navigation-drawer v-model="drawer">
      <div class="px-2 py-2">
        <v-text-field v-model="systemExpecte" type="text" placeholder="助理期待" />
      </div>

      <div class="px-2 py-2">
        <v-row>
          <v-col>
            <v-checkbox v-model="moreChat" class="mx-2" label="多轮"></v-checkbox>
          </v-col>
          <v-col>
            <v-text-field label="轮数" v-model="chatCount"></v-text-field>
          </v-col>
        </v-row>
      </div>

      <div class="px-2 py-2">
        <v-btn block variant="outlined" size="large" color="red">重置对话</v-btn>
      </div>

      <div class="px-2 py-2">
        <v-btn block variant="outlined" prepend-icon="add" size="large" @click="createNewConversion()">
          New conversation
        </v-btn>
        <v-list>
          <v-list-item v-for="conversation in conversations" :key="conversation.id" :title="conversation.topic"
            active-color="primary" @click="openConversationMessages(conversation)"></v-list-item>
        </v-list>
      </div>
      ...
      <template v-slot:append>
        <div class="px-1">
          ...
          <v-list>
            ...
            <v-list-item rounded="xl" prepend-icon="help_outline" :title="$t('feedback')" @click="feedback"></v-list-item>
          </v-list>
        </div>
      </template>
    </v-navigation-drawer>
    ...
    <v-app-bar class="d-lg-none">
      ...
      <v-menu>
        <template v-slot:activator="{ props }">
          <v-btn v-bind="props" icon="help_outline" title="Feedback"></v-btn>
        </template>
        <v-list>
